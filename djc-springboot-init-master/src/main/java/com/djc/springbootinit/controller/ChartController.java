...
import com.djc.springbootinit.bizmq.BiMessageProducer;
...
import com.djc.springbootinit.manager.RedisLimiterManager;
import com.djc.springbootinit.model.dto.chart.*;
import com.djc.springbootinit.model.entity.Chart;
import com.djc.springbootinit.model.entity.User;
import com.djc.springbootinit.model.vo.BiResponse;
import com.djc.springbootinit.service.ChartService;
import com.djc.springbootinit.service.UserService;
import com.djc.springbootinit.utils.ExcelUtils;
...
public class ChartController {
     ...
     */
    @PostMapping("/gen")
    public Object genChartByAI(@RequestPart("file") MultipartFile multipartFile,
        ...
        ThrowUtils.throwIf(size > MAX_FILE_SIZE, ErrorCode.PARAMS_ERROR, "文件过大");
        // 校验文件后缀
        String originalFilename = multipartFile.getOriginalFilename();
        String suffix = FileUtil.getSuffix(originalFilename);
        final List<String> allowSuffix = Arrays.asList("xls", "xlsx");
        ThrowUtils.throwIf(!allowSuffix.contains(suffix), ErrorCode.PARAMS_ERROR, "文件类型错误");
        User loginUser = userService.getLoginUser(request);
        try {
            //限流
        ...
        List<ChatCompletionChoice> result = aiManager.doChat(userInput.toString());
        ThrowUtils.throwIf(result == null || result.size() == 0, ErrorCode.SYSTEM_ERROR, "AI 生成失败");
        //由于AI返回的结果是一个列表，并且非流式返回，所以List只有一个元素
        String retResult = String.valueOf(result.get(0).getMessage().getContent());
        String[] splits = retResult.split("【【【");
        if (splits.length < 3) {
            throw new BusinessException(ErrorCode.SYSTEM_ERROR, "AI 生成错误");
        }
        String genChart = splits[1].trim(); //前端代码（Echarts V5）
        String genResult = splits[2].trim();//分析结果
        // 插入到数据库
        Chart chart = new Chart();
        chart.setName(name);
        chart.setGoal(goal);
